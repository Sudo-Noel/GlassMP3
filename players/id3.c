...
void readTagData(int fp, int tagLength, int maxTagLength, char *tagValue){
    ...
    if ( utf8Tag == NULL || !strlen(utf8Tag)) {
        utf8Tag = miniConvSubtitleConv(carattere, "ISO-8859-1");
        if (utf8Tag && strlen(utf8Tag)) {
            strncpy( tagValue, utf8Tag, maxTagLength - 1 );
            tagValue[maxTagLength - 1] = '\0';
        } else {
            for (i=0; i<tagLength && count < maxTagLength - 1; i++){
   ...
   //if(ID3v2(mp3path) == 2) {
      ...
      while (size != 0) {
         ...
         else if(!strncmp("PIC",tag,3)) /* Picture */
         {
            ...
            tag_length = tag_length - (id3tag->ID3EncapsulatedPictureOffset - sceIoLseek(fp, 0, PSP_SEEK_CUR));
...
int ParseID3v2(const char *mp3path, struct ID3Tag *id3tag)
{
   ...
   /*if(*id3tag->ID3Title == 0) {
      char *filename = strrchr(mp3path,'/');
      if (filename) {
          strncpy(id3tag->ID3Title, filename + 1, sizeof(id3tag->ID3Title) - 1);
      } else {
          strncpy(id3tag->ID3Title, mp3path, sizeof(id3tag->ID3Title) - 1);
      }
      id3tag->ID3Title[sizeof(id3tag->ID3Title) - 1] = '\0';
      if (*strrchr(id3tag->ID3Title,'.') != 0) *strrchr(id3tag->ID3Title,'.') = 0;
   }*/
   return 0;
}

static void ensureUTF8(char *dest, int max_len) {
    char *utf8Tag = miniConvSubtitleConv((unsigned char*)dest, "ISO-8859-1");
    if (utf8Tag && strlen(utf8Tag)) {
        strncpy(dest, utf8Tag, max_len - 1);
        dest[max_len - 1] = '\0';
    }
}
...
int ParseID3v1(const char *mp3path, struct ID3Tag *id3tag){
    ...
    if (strstr(id3buffer,"TAG") != NULL){
        ...
        ensureUTF8(id3tag->ID3Title, sizeof(id3tag->ID3Title));
        ensureUTF8(id3tag->ID3Artist, sizeof(id3tag->ID3Artist));
        ensureUTF8(id3tag->ID3Album, sizeof(id3tag->ID3Album));
     }else{
        sceIoClose(id3fd);
        return -1;
     }
...
int ParseID3(char *mp3path, struct ID3Tag *target)
{
    ...
    if (!strlen(target->ID3Title)) {
        ...
        ensureUTF8(target->ID3Title, sizeof(target->ID3Title));
    }
    return 0;
}
