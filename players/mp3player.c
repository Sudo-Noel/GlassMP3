...
// Fix for inline assembly constraint errors with newer GCC MIPS
#undef mad_f_mul
#define mad_f_mul(x, y) (mad_fixed_t)((((long long)(x)) * (y)) >> MAD_F_FRACBITS)
...
int fillFileBuffer() {
	...
	while (bytesToFill > 0){
		const unsigned int bytesRead = sceIoRead(MP3_fd, bufferPos, bytesToFill);
