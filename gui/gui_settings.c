...
int changeSettingVal(int index, int delta){
    ...
    switch(index){
        ...
        case 8:
            ...
            break;
        case 9:
            if (userSettings->CLOCK_GUI + delta <= 333 && userSettings->CLOCK_GUI + delta >= getMinCPUClock()) {
            ...
            break;
        case 10:
            if (userSettings->CLOCK_MP3 + delta <= 333 && userSettings->CLOCK_MP3 + delta >= getMinCPUClock()) {
                userSettings->CLOCK_MP3 += delta;
                MP3_defaultCPUClock = userSettings->CLOCK_MP3;
            }
            break;
        case 11:
            if (userSettings->CLOCK_MP3ME + delta <= 333 && userSettings->CLOCK_MP3ME + delta >= getMinCPUClock()) {
                userSettings->CLOCK_MP3ME += delta;
                MP3ME_defaultCPUClock = userSettings->CLOCK_MP3ME;
            }
            break;
        case 12:
            if (userSettings->CLOCK_OGG + delta <= 333 && userSettings->CLOCK_OGG + delta >= getMinCPUClock()) {
                userSettings->CLOCK_OGG += delta;
                OGG_defaultCPUClock = userSettings->CLOCK_OGG;
            }
            break;
        case 13:
            if (userSettings->CLOCK_FLAC + delta <= 333 && userSettings->CLOCK_FLAC + delta >= getMinCPUClock()) {
                userSettings->CLOCK_FLAC += delta;
                FLAC_defaultCPUClock = userSettings->CLOCK_FLAC;
            }
            break;
        case 14:
            if (userSettings->CLOCK_AA3 + delta <= 333 && userSettings->CLOCK_AA3 + delta >= getMinCPUClock()) {
                userSettings->CLOCK_AA3 += delta;
                AA3ME_defaultCPUClock = userSettings->CLOCK_AA3;
            }
            break;
        case 15:
            if (userSettings->CLOCK_WMA + delta <= 333 && userSettings->CLOCK_WMA + delta >= getMinCPUClock()) {
