...
static float scrollArtistX = -1.0f;
static float scrollAlbumX = -1.0f;
static int coverArtAlpha = 0;
...
int drawFileInfo(struct fileInfo *info, struct libraryEntry *libEntry, char *trackMessage){
    ...
    if (titleWidth > 0 && oslGetStringWidth(info->artist) > titleWidth){
        if (fontNormal->fontType == OSL_FONT_INTRA){
            setFontStyle(fontNormal, defaultTextSize, RGBA(tempColor[0], tempColor[1], tempColor[2], tempColor[3]), RGBA(tempColorShadow[0], tempColorShadow[1], tempColorShadow[2], tempColorShadow[3]), INTRAFONT_SCROLL_SEESAW);
            if (scrollArtistX < 0)
                scrollArtistX = tempPos[0];
            scrollArtistX = oslIntraFontPrintColumn(fontNormal, scrollArtistX, tempPos[1], titleWidth, info->artist);
            setFontStyle(fontNormal, defaultTextSize, RGBA(tempColor[0], tempColor[1], tempColor[2], tempColor[3]), RGBA(tempColorShadow[0], tempColorShadow[1], tempColorShadow[2], tempColorShadow[3]), INTRAFONT_ALIGN_LEFT);
        }else
            oslDrawStringLimited(tempPos[0], tempPos[1], titleWidth, info->artist);
    }else
        oslDrawString(tempPos[0], tempPos[1], info->artist);

    skinGetPosition("POS_ALBUM_VALUE", tempPos);
    if (titleWidth > 0 && oslGetStringWidth(info->album) > titleWidth){
        if (fontNormal->fontType == OSL_FONT_INTRA){
            setFontStyle(fontNormal, defaultTextSize, RGBA(tempColor[0], tempColor[1], tempColor[2], tempColor[3]), RGBA(tempColorShadow[0], tempColorShadow[1], tempColorShadow[2], tempColorShadow[3]), INTRAFONT_SCROLL_SEESAW);
            if (scrollAlbumX < 0)
                scrollAlbumX = tempPos[0];
            scrollAlbumX = oslIntraFontPrintColumn(fontNormal, scrollAlbumX, tempPos[1], titleWidth, info->album);
            setFontStyle(fontNormal, defaultTextSize, RGBA(tempColor[0], tempColor[1], tempColor[2], tempColor[3]), RGBA(tempColorShadow[0], tempColorShadow[1], tempColorShadow[2], tempColorShadow[3]), INTRAFONT_ALIGN_LEFT);
        }else
            oslDrawStringLimited(tempPos[0], tempPos[1], titleWidth, info->album);
    }else
...
int drawCoverArt(){
    ...
    if (coverArtAlpha < 255) {
        coverArtAlpha += 10;
        if (coverArtAlpha > 255) coverArtAlpha = 255;
    }
    
    oslSetAlpha(OSL_FX_ALPHA, coverArtAlpha);
    if (coverArt)
...
int drawProgressBar(float perc){
    ...
    int barWidth = (int)((float)progress->sizeX / 100.00 * perc);
    if (barWidth > 0) {
        oslSetImageTileSize(progress, 0, 0, barWidth, progress->sizeY);
...
int playFile(char *fileName, char *trackMessage, int index, double startFilePos){
    ...
    if (!libEntry) {
        static struct libraryEntry dummyEntry;
        memset(&dummyEntry, 0, sizeof(dummyEntry));
        libEntry = &dummyEntry;
    }
