...
void addDirectoryToPlaylist(char *dirName, char *dirNameShort){
	...
	if (result == 0){
		...
		for (i = 0; i < dirToAdd.number_of_directory_entries; i++){
			...
			strncpy(fileToAdd, dirNameShort, sizeof(fileToAdd) - 1);
            fileToAdd[sizeof(fileToAdd) - 1] = '\0';
			if (strlen(fileToAdd) > 0 && fileToAdd[strlen(fileToAdd)-1] != '/')
                if (strlen(fileToAdd) < sizeof(fileToAdd) - 1)
				    strcat(fileToAdd, "/");
            int remaining = sizeof(fileToAdd) - strlen(fileToAdd) - 1;
            if (remaining > 0)
			    strncat(fileToAdd, dirToAdd.directory_entry[i].d_name, remaining);
