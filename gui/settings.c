...
int SETTINGS_load(char *fileName){
	...
	// Fallback to LightGlass if the old default skin was set
	if (strcmp(localSettings.skinName, "default") == 0) {
	    strcpy(localSettings.skinName, "LightGlass");
	}
	
	// Force FLAC clock to 333 to fix slow decoding
	if (localSettings.CLOCK_FLAC < 333) {
	    localSettings.CLOCK_FLAC = 333;
	}
